...
class QuestionnaireRepondu
{
    ...
    private function gestionAffichageModeLivraison(Questionnaire $questionnaire)
    {
        ...
        $livraisonTypeAucun = $this->em->getRepository('SceauBundle:LivraisonType')->aucun();
        foreach ($questions as $question) {
    ...
    {
        $questionnaireDemande = $this->em->getRepository('SceauBundle:Questionnaire')
        ...
        if ($questionnaireDemande->getQuestionnaireLie() != null) {
            ...
            $questionnaire1 = $this->em->getRepository('SceauBundle:Questionnaire')
            ...
            $questionnaire2_id = $this->em->getRepository('SceauBundle:Questionnaire')
                ->findBy(array('questionnaireLie' => $questionnaireDemande));

            if ($questionnaire2_id) {
                $questionnaire2 = $this->em->getRepository('SceauBundle:Questionnaire')
